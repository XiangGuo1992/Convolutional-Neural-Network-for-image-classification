...
def create_train_data():
    ...
    np.save('C:\\github\\action_detection_test\\frames\\result\\{}_train_data.npy'.format(subject), training_data)
...
for train in tqdm(folders): 
    os.chdir('C:\\github\\action_detection_test\\frames\\Labeled_data')
    TRAIN_DIR = os.path.join(os.getcwd(),train)
    subject = TRAIN_DIR.split('\\')[-1]
    #TEST_DIR = os.path.join(os.getcwd(),'testing')
    
    
    MODEL_NAME = '{}-{}-{}.model'.format(subject,LR, '6conv-basic') # just so we remember which saved model is which, sizes must match
    ...
    os.chdir('C:\\github\\action_detection_test\\frames\\result')
    ...
    model = tflearn.DNN(convnet, tensorboard_dir='C:\\github\\action_detection_test\\frames\\result\\tensorboard\\{}\\log'.format(subject))
    ...
    model.save('C:\\github\\action_detection_test\\frames\\result\\model\\'+ subject + '\\'+MODEL_NAME)
...
csv_out.columns = ['Subject','Accuracy']
